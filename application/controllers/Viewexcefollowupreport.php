...
class Viewexcefollowupreport extends CI_Controller {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('C2', 'Business Promotion Followup Data');
    $objPHPExcel->getActiveSheet()->getStyle('C2')->getFont()->setSize(16);
    $objPHPExcel->getActiveSheet()->getStyle('C2')->getFont()->setBold(true); // 
    
    if (!empty($_GET['from_followup_date'])|| !empty($_GET['to_followup_date'])) {
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'Business Promotion Followup Report for the period "'.$_GET['from_followup_date'].'" to "'.$_GET['to_followup_date'].'"'); 
      } else { 
        $objPHPExcel->getActiveSheet()->SetCellValue('C3', 'Business Interaction Followup Report for the Selected All period'); 
      }
     $objPHPExcel->getActiveSheet()->getStyle('C3')->getFont()->setSize(12);
     $objPHPExcel->getActiveSheet()->getStyle('C3')->getFont()->setBold(true);  

	   $result = $this->Interaction_master->getFollowupdataSearch($_GET);
	   //print_r($result);exit;
     $k=4;
     $objPHPExcel->getActiveSheet()->SetCellValue('A'.$k,'Sr.No.');
     $objPHPExcel->getActiveSheet()->SetCellValue('B'.$k,'Client Name');
     $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,'Followup');
     $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,'Followup Date');
     $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,'Followup Remarks');
     $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,'Employee Name');
     

     $objPHPExcel->getActiveSheet()->getStyle('A'.$k.':F'.$k)->getFont()->setSize(12);
     $objPHPExcel->getActiveSheet()->getStyle('A'.$k.':F'.$k)->getFont()->setBold(true);
    ...
    foreach ($result as $interaction_data) {
        ...
        if ($interaction_data['followup_flag']==1) { $followup_flag = 'Yes'; } else { $followup_flag = 'No'; }
        if ($interaction_data['followup_date']!='') { $followup_date = date('d-m-Y',strtotime($interaction_data['followup_date']));} else {$followup_date = '';}

        $objPHPExcel->getActiveSheet()->SetCellValue('A'.$k,$k-4);
        $objPHPExcel->getActiveSheet()->SetCellValue('B'.$k,$interaction_data['client_name']);

        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$k,$followup_flag);
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$k,$followup_date);
        $objPHPExcel->getActiveSheet()->SetCellValue('E'.$k,$interaction_data['followup_text']);
        $objPHPExcel->getActiveSheet()->SetCellValue('F'.$k,$interaction_data['emp_first_name']." ".$interaction_data['emp_last_name']);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A4:F'.($k-1))->applyFromArray($BStyle);
    $objPHPExcel->getActiveSheet()->getStyle('A4:F'.($k-1))->getAlignment()->setWrapText(true); 
    $objPHPExcel->getActiveSheet()->getStyle('A4:F'.($k-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
    

      	//echo $pdf1;exit;
    $filename = "Promotion_Client_Followup_Report_". date("Y-m-d-H-i-s").".xlsx";
